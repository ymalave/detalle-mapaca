    ...
    <div x-data="data()">
        <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
            <div class="mb-2 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="flex-shrink-0">
                    <a href="{{ route('configuracion.proveedor.create') }}"
                ...
                <div class="flex flex-col">
                    ...
                    <template x-for="(proveedor, key) in filteredProveedores()" :key="key">
                        <div class="grid grid-cols-3 border-b border-stroke dark:border-strokedark sm:grid-cols-4">
                            ...
                            <div class="flex items-left justify-left p-2.5 xl:p-5">
                                <p class="font-medium text-black dark:text-white" x-text="proveedor.nombre"></p>
                            </div>

                            <div class="flex items-center justify-center p-2.5 xl:p-5">
                                <p class="font-medium text-black dark:text-white" x-text="proveedor.telefono"></p>
                            </div>


                            <div class="hidden items-center justify-center p-2.5 sm:flex xl:p-5">
                                <div class="flex items-center space-x-3.5">
                                    <a :href="`{{ route('configuracion.proveedor.edit', '') }}/${proveedor.cod_proveedor}`"
                                        class="hover:text-primary">
                                        <i class="fa-regular fa-pen-to-square"></i>
                                    </a>
                                    <button class="hover:text-primary" @click.prevent="deleteItem(proveedor)">
        ...
        @push('js')
            <script>
                function data() {
                    return {
                        proveedores: @json(@old('provvedores', $proveedores ?? [])),
                        search: '',
                        currentPage: 1,
                        itemsPerPage: 8,
                        ...
                        filteredProveedores() {
                            const filtered = this.proveedores.filter(proveedor => {
                                return proveedor.rif.toLowerCase().includes(this.search.toLowerCase()) ||
                                    proveedor.nombre.toLowerCase().includes(this.search.toLowerCase()) ||
                                    proveedor.telefono.includes(this.search);
                            });
                        ...
                        getTotalPages() {
                            return Math.ceil(this.proveedores.filter(proveedor => {
                                return proveedor.rif.toLowerCase().includes(this.search.toLowerCase()) ||
                                    proveedor.nombre.toLowerCase().includes(this.search.toLowerCase()) ||
                                    proveedor.telefono.toLowerCase().includes(this.search.toLowerCase());
                        ...
                        deleteItem(proveedor) {
                            const route = `{{ route('configuracion.proveedor.delete', '') }}/${proveedor.cod_proveedor}`;
                            Swal.fire({
                                icon: 'warning',
                                title: '¿Estas Seguro?',
