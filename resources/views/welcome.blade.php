    ...
    <body class="font-sans antialiased">
        ...
        <div id="default-carousel" class="relative w-full" data-carousel="slide">
            ...
            <div class="relative h-56 overflow-hidden rounded-lg md:h-96">
                <!-- Iterar sobre el array de marcas -->
                @foreach ($marcas as $index => $marca)
                    <div class="hidden duration-700 ease-in-out bg-gray-50" data-carousel-item>
                        <img src="{{ asset('images/' . strtolower($marca) . '.jpg') }}" class="absolute block w-full h-full object-contain -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2" alt="{{ $marca }}">
                    </div>
                @endforeach
            </div>
            ...
            <div class="absolute z-30 flex -translate-x-1/2 bottom-5 left-1/2 space-x-3 rtl:space-x-reverse">
                @foreach ($marcas as $index => $marca)
                    <button type="button" class="w-3 h-3 rounded-full bg-gray-500" aria-current="{{ $index === 0 ? 'true' : 'false' }}" aria-label="Slide {{ $index + 1 }}" data-carousel-slide-to="{{ $index }}"></button>
                @endforeach
            </div>
